...
build_release() {
  ...
  ( cd "${temporary}/backend" && grep -vE '^-e[[:space:]]+\.\.$' requirements.txt > .release-requirements.txt && run_clean "${temporary}/backend/.venv/bin/python" -m pip install --requirement .release-requirements.txt && mkdir .release-wheel && run_clean "${temporary}/backend/.venv/bin/python" -m pip wheel --no-deps --wheel-dir .release-wheel .. && run_clean "${temporary}/backend/.venv/bin/python" -m pip install --no-deps .release-wheel/xraylarch-*.whl && rm -rf .release-requirements.txt .release-wheel && run_clean "${temporary}/backend/.venv/bin/python" -m pip check && run_clean "${temporary}/backend/.venv/bin/python" -m pip freeze --all > pip-freeze.txt ) || return 1
