...
test('native detector probe previews measured counts, corrects type, and survives PRJ exchange', async ({ page }, info) => {
  ...
  await expect(page.getByRole('region', { name: 'Single spectrum viewer', exact: true }).getByRole('radio', { name: 'Detector signal' })).toBeChecked()
  await expect(page.getByRole('region', { name: 'Single spectrum viewer', exact: true }).getByRole('radio', { name: 'Detector signal' })).toBeDisabled()
  ...
  await page.getByRole('region', { name: 'Single spectrum viewer', exact: true }).getByRole('tab', { name: 'k EXAFS', exact: true }).click()
  await expect(page.getByText('No data in this plot space', { exact: true })).toBeVisible()
  await page.getByRole('region', { name: 'Single spectrum viewer', exact: true }).getByRole('tab', { name: 'E Energy', exact: true }).click()
