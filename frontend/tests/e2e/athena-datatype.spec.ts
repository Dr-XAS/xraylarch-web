...
test('real copper type correction: current, frozen, marked, all, drafts and plot arrays', async ({ page }, info) => {
  ...
  await command(page, () => page.getByLabel(`Mark ${reference.label}`, { exact: true }).click())
  ...
  expect(saved.groups.map((g: { data_type: string }) => g.data_type)).toEqual(['xanes', 'xanes', 'mu', 'mu'])
...
test('normalized XANES survives downloaded .prj and reopened plots', async ({ page }, info) => {
  test.setTimeout(90000)
  await page.goto('/')
  await command(page, () => page.getByRole('button', { name: 'Load copper examples', exact: true }).click())
  ...
  const savedIds = new Set(saved.groups.map((group: { id: string }) => group.id))
  ...
  // The import panel may append grouped spectra to this project; select the imported copy by identity.
  const next = await accepted.json(); const actual = next.groups.find((group: { id: string; label: string }) => !savedIds.has(group.id) && group.label === saved.groups[0].label)!
