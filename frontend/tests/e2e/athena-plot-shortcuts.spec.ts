...
for (const mobile of [false, true]) test((mobile ? 'mobile' : 'desktop') + ' original Fe PRJ and measured detector scan: seven shortcuts and project roundtrip', async ({ page }, info) => {
  ...
  await close(dialog)
  await page.getByRole('region', { name: 'Multiple spectra viewer', exact: true }).getByRole('tab', { name: /^E(?: Energy)?$/ }).click()
  await page.getByRole('region', { name: 'Multiple spectra viewer', exact: true }).getByRole('radio', { name: 'Derivative dμ/dE', exact: true }).check()
  dialog = await open(page)
