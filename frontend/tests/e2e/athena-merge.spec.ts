...
for(const mobile of [false,true])test(`${mobile?'mobile':'desktop'} measured Fe merge, linked references, saved spread plots and bare PRJ`,async({page},info)=>{
  ...
  const download=page.waitForEvent('download');await page.getByRole('button',{name:'Save project',exact:true}).click();const path=info.outputPath('merged-native.prj');await(await download).saveAs(path)
