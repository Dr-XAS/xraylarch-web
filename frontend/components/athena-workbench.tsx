...
import type { Space } from "./artefact-viewers/athena-plot"
...
function AthenaWorkbenchContent({ session }: { session: AthenaSession }) {
  ...
  function renderSpectrumViewer(viewer: "single" | "multiple") {
    ...
    return <AthenaSpectrumViewer viewer={viewer} state={state}
      ...
      onPickX={viewer === "single" && pick ? (x, space) => pluck(x, space, pick) : undefined}
      pickPrompt={viewer === "single" && pick && <div className="ath-pick-prompt" aria-live="polite"><span>Picking <strong>{pick.label}</strong> for {active?.label}. Click a spectrum in the {pick.space} plot{pick.relative && `; E − E₀ uses ${pick.e0} eV`}. You can also type the field value. Changes process automatically.</span><button onClick={cancelPick}>Cancel pick <kbd>Esc</kbd></button></div>}
      onExport={viewer === "single" && active && project && can("export") ? () => {
        if (!parameterActionBlocked()) void download(`/api/athena/projects/${project.id}/groups/${active.id}/export?space=${state.space}`, `${active.label}.csv`)
      } : undefined} />
  }
