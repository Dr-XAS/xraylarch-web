...
function NumberField({ label, value, onChange, unit, effective, optional = false, step = "any", pick }: {
  ...
  const input = <label className="ath-field"><span>{label}{unit && <small>{unit}</small>}</span><input type="number" step={step} value={value ?? ""} placeholder={optional ? "Auto" : ""} onChange={e => onChange(e.target.value === "" ? null : Number(e.target.value))} />{optional && value === null && typeof effective === "number" && <em>Auto: {effective.toFixed(3).replace(/\.?0+$/, "")}</em>}{optional && value !== null && typeof effective === 'number' && value !== effective && <em>Used in saved result: {Number(effective.toPrecision(10))}</em>}</label>
...
export function AthenaWorkbench() {
  ...
  return <main className="ath-app" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); if (!busy && !registryPending && project) void queueFiles(Array.from(e.dataTransfer.files)) }}>
    ...
    {modal && modal !== "difference" && modal !== "rebin" && modal !== "dispersive" && modal !== 'multi_electron' && modal !== 'smooth' && modal !== 'convolve' && modal !== 'deglitch' && modal !== 'truncate' && modal !== 'calibrate' && modal !== 'align' && toolTitles[modal] && <Modal title={toolTitles[modal]} close={() => { if (!busy) setModal(null) }}><div className="ath-modal-body"><p className="ath-tool-target">Current group <strong>{active?.label}</strong></p>
