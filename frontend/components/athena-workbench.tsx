...
function NumberField({ label, value, onChange, unit, effective, optional = false, step = "any", pick, showAutomaticValue = false }: {
  label: string; value: number | null; onChange: (v: number | null) => void; unit?: string
  effective?: unknown; optional?: boolean; step?: string; pick?: ReactNode; showAutomaticValue?: boolean
}) {
  ...
  const input = <label className="ath-field"><span>{label}{unit && <small>{unit}</small>}</span><input type="number" step={step} value={displayedValue} placeholder={optional ? "Auto" : ""} onChange={e => {
...
export function AthenaWorkbench() {
  ...
  return <main className="ath-app" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); if (!busy && !registryPending && project) void queueFiles(Array.from(e.dataTransfer.files)) }}>
    <header className="ath-header"><div className="ath-brand"><span className="ath-logo"><Activity size={25} /></span><div><h1>ATHENA <span>WEB</span></h1><p>X-ray absorption spectroscopy</p></div></div>
      <nav aria-label="Main menu">{["File", "Edit", "Group", "Energy", "Process", "Analysis"].map(label => <div className="ath-menu-wrap" key={label}><button aria-expanded={menu === label} onClick={() => setMenu(menu === label ? "" : label)}>{label}<ChevronDown size={12} /></button>{menu === label && <div className="ath-menu" onKeyDown={e => { if (e.key === "Escape") setMenu("") }}>
    ...
    <ResizableAthenaWorkspace
      groups={<aside id="athena-data-groups" className="ath-groups"><div className="ath-panel-heading"><h2>Data groups <span>{project?.groups.length ?? 0}</span></h2><button aria-label="Import spectra" disabled={!project || !!busy} onClick={() => setModal("import")}><Plus size={17} /></button></div><label className="ath-search"><Search size={14} /><input aria-label="Search groups" placeholder="Find a spectrum…" value={search} onChange={e => setSearch(e.target.value)} /></label>
